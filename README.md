...
본 레포지토리는 **Java Stream 사용 시 자주 발생하는 실수와 그 원인, 개선 방법**을  
...
List<String> filtered =
    names.stream()
         ...
         .toList();
✔️ Stream 내부에서는 불변 처리(Immutable) 유지
✔️ 결과는 새로운 컬렉션으로 생성
...
names.stream()
     ...
     .toList();
✔️ 실제 테스트 결과 약 7~10배 수행 시간 차이 발생
✔️ 가독성보다 성능이 중요한 경우 연산 통합 고려
...
✔️ isPresent() 또는 orElse() 사용 권장
✔️ orElse() 사용 시 성능 차이는 유의미하지 않음
...
numbers.parallelStream()
       .forEach(n -> results.add(n * 2));
✔️ 멀티스레드 환경에서는 Thread-safe 자료구조 사용 필수
✔️ 가능하면 collect() 기반의 불변 처리 권장
